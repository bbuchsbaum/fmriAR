...
test_that("run pooling applies the matching correction to every run", {
  ...
  plain <- fit_noise(R, runs = runs, method = "ar", p = 2L, pooling = "run")
  ...
  for (k in names(plan$phi)) {
    expect_equal(plan$phi[[k]], phi, tolerance = 0.08, info = k)
    # The correction must land closer to the truth than the uncorrected fit,
    # in every run; an absolute tolerance alone cannot tell the two apart.
    expect_lt(sum(abs(plan$phi[[k]] - phi)), sum(abs(plain$phi[[k]] - phi)))
