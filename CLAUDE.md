...
fmriAR is an R package that provides fast AR/ARMA prewhitening for fMRI
design and data matrices. It uses C++ (via Rcpp and RcppArmadillo) for
...
1.  **Noise Model Estimation** (`R/fit_and_apply.R`)
    - [`fit_noise()`](https://bbuchsbaum.github.io/fmriAR/reference/fit_noise.md):
      Estimates AR/ARMA parameters from residuals
...
2.  **Whitening Application** (`R/fit_and_apply.R`)
    - [`whiten_apply()`](https://bbuchsbaum.github.io/fmriAR/reference/whiten_apply.md):
      Applies prewhitening transformation to design matrix X and data Y
    - Returns whitened versions suitable for standard linear model
      fitting
    - Handles multi-run fMRI data with proper boundary conditions
3.  **C++ Backend** (`src/`)
    - `arma_whiten_inplace()`: Core whitening implementation using
      RcppArmadillo
...
4.  **Statistical Utilities**
    - [`sandwich_from_whitened_resid()`](https://bbuchsbaum.github.io/fmriAR/reference/sandwich_from_whitened_resid.md)
      (`R/sandwich.R`): Computes robust standard errors
    - [`acorr_diagnostics()`](https://bbuchsbaum.github.io/fmriAR/reference/acorr_diagnostics.md)
      (`R/acorr.R`): Autocorrelation diagnostics for residuals
    - `pacf_cpp()` (`R/pacf_helpers.R`): Partial autocorrelation
      computations
...
2.  Estimate noise model from residuals using
    [`fit_noise()`](https://bbuchsbaum.github.io/fmriAR/reference/fit_noise.md)
3.  Apply whitening to both design matrix and data using
    [`whiten_apply()`](https://bbuchsbaum.github.io/fmriAR/reference/whiten_apply.md)
...
- Optional: OpenMP support (uncomment in `src/Makevars` for parallel
  processing)
